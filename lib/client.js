window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		//#endregion
		//#region src/client/AgentList.tsx
		...
		function AgentList({ parentId, props, ancestors, managed = false, visibleIds }) {
			...
			(0, react.useEffect)(() => {
				...
				return () => {
					setSubagentCatalogOpen(parentId, false);
				};
			}, [
				parentId,
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "dsk-agent-list",
				children: [
					catalog === void 0 || catalog.state === "loading" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						role: "status",
						children: t("agents.loading")
					}) : catalog.state === "error" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						role: "alert",
						children: catalog.error?.message ?? t("unavailable")
					}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("agents.scope", {
						running: children.filter((child) => child.activity === "running").length,
						total: children.length
					}) }),
					catalog?.state === "ready" && catalog.entries.length === 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("agents.empty") }),
					catalog?.entries.map((entry) => {
						if (entry.kind === "diagnostic") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: t("agents.diagnostic", { reason: entry.reason }) }, entry.id);
						if (visibleIds !== void 0 && !visibleIds.has(entry.id)) return null;
						...
						const open = visibleIds !== void 0 || expanded.has(entry.id);
						return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "dsk-agent-row",
							"data-activity": pending.has(entry.id) ? "waiting" : entry.activity,
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									className: "dsk-agent-name",
									type: "button",
									"aria-label": t("agents.open", { name: title }),
									onClick: () => {
										props.openSubagent({
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: t(pending.has(entry.id) ? "agents.waiting" : entry.activity === "running" ? "agents.running" : "agents.inactive") }),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: "dsk-agent-mode",
									children: t(`agents.mode.${entry.mode}`)
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", {
									className: "dsk-agent-details",
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("summary", { children: t("agents.details") }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: model?.model === void 0 ? t("agents.noModel") : t("agents.nextModel", { model: [model.provider, model.model].filter(Boolean).join("/") }) }),
										usage !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("card.tokens", { count: String(usage.uncachedInputTokens + usage.cacheReadTokens + usage.cacheWriteTokens + usage.outputTokens) }) }),
										steps !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("card.steps", { count: steps }) }),
										used !== void 0 && pressure?.contextWindow !== void 0 && pressure.contextWindow > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("card.context", { percent: Math.round(Math.min(100, used / pressure.contextWindow * 100)) }) }),
										typeof preset === "string" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("agents.preset", { preset }) }),
										goal !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", { children: [
											t("agents.goal", { objective: goal.objective }),
											" · ",
											t(`goal.${goal.phase}`)
										] }),
										goal?.blockedReason !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
											className: "dsk-failure",
											children: goal.blockedReason.message
										})
									] })]
								}),
								entry.hasChildren && !ancestors.includes(entry.id) && (visibleIds === void 0 || sessions.subagentsByParent[entry.id]?.entries.some((child) => visibleIds.has(child.id))) && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [visibleIds === void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									"aria-expanded": open,
									onClick: () => {
									...
									children: t(open ? "agents.hide" : "agents.show")
								}), open && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AgentList, {
									parentId: entry.id,
									props,
					...
					!managed && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						type: "button",
						onClick: () => {
							props.refreshSubagents(parentId).catch((cause) => {
								props.actions.setError(cause instanceof Error ? cause.message : String(cause));
							});
						},
						children: t("agents.retry")
					})
				]
			});
		}
		//#endregion
		//#region src/client/agent-tree.ts
		/** Collect reachable catalogs and retain ancestors of matching descendants for filtered trees. */
		function projectAgentTree(root, catalogs, waiting, filter = "all") {
			...
			for (const id of matches) {
				...
				while (parent !== void 0 && parent !== root && !visible.has(parent)) {
					visible.add(parent);
		...
		//#endregion
		//#region src/client/AgentTree.tsx
		...
		function AgentTree({ parentId, props }) {
			...
			const observed = (0, react.useRef)(/* @__PURE__ */ new Set());
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "dsk-agents",
				onClick: (event) => {
					event.stopPropagation();
				},
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "dsk-agent-summary",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: t(tree.incomplete ? "agents.totalPartial" : "agents.total", {
							total: tree.total,
					...
					tree.incomplete && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						role: "status",
						children: t(tree.failed ? "agents.partialError" : "agents.loading")
					}),
					open && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "dsk-agent-filters",
							children: [
								"all",
								"running",
								"waiting"
								...
								children: t(`agents.filter.${value}`)
							}, value))
						}),
						filter !== "all" && tree.visible.size === 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("agents.noMatches") }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "dsk-agent-scroll",
							role: "region",
							"aria-label": t("agents.hierarchy"),
							tabIndex: 0,
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AgentList, {
								parentId,
								props,
								ancestors: [parentId],
								managed: true,
								visibleIds: filter === "all" ? void 0 : tree.visible
							})
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							onClick: () => {
								Promise.all([...tree.parents].map((id) => props.refreshSubagents(id))).catch((cause) => {
									props.actions.setError(cause instanceof Error ? cause.message : String(cause));
								});
							},
							children: t("agents.refreshTree")
						})
					] })
				]
			});
		...
		*/
		...
		const TaskCard = (0, react.memo)(function TaskCard({ card, settings, props, children }) {
			const { t } = props;
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							card.provider !== void 0 && card.model !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("agents.nextModel", { model: `${card.provider}/${card.model}` }) }),
		...
		function BoardColumnView({ column, cards, allCards, settings, props }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", { children: props.t(`column.${column}`) }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: cards.length })] }),
					cards.length === 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						className: "dsk-column-empty",
						children: props.t("column.empty")
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [visibleCards.map((card) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(TaskCard, {
							...
							settings,
							props,
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AgentTree, {
								parentId: card.id,
								props
							})
		...
		function Columns({ cards, settings, props }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				"data-focused": focused !== void 0 || void 0,
				children: (focused === void 0 ? BOARD_COLUMNS : [focused]).map((column) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(BoardColumnView, {
					column,
					allCards: cards,
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", {
								className: "dsk-view-options",
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("summary", { children: props.t("filters.options") }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									className: "dsk-options-grid",
									children: [
		...
		const en = {
			...
			"filters.options": "View options",
			"agents.details": "Model & usage",
		...
		const zh = {
			...
			newTask: "新建任务",
			diagnostics: "诊断",
			"filters.options": "视图选项",
			"agents.details": "模型与用量",
			"agents.hierarchy": "Agent 层级",
			"column.empty": "没有任务",
...
.dsk-root{position:fixed;inset:0;z-index:1000;display:flex;flex-direction:column;overflow:hidden;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);font-family:var(--dsw-font-family);font-size:14px}
...
.dsk-root button,.dsk-sidebar-action{border:1px solid var(--dsw-alias-border-l2);border-radius:8px;background:var(--dsw-alias-bg-layer-2);cursor:pointer}
...
.dsk-topbar{flex-shrink:0;display:flex;align-items:center;justify-content:space-between;gap:16px;padding:16px 24px;border-bottom:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1)}
.dsk-topbar-lead{display:flex;flex:none;align-items:center;gap:14px;min-width:0}
.dsk-topbar-title{min-width:0}
.dsk-topbar h1{margin:0;font-size:20px;letter-spacing:-.5px;line-height:28px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.dsk-stats{flex-shrink:0;display:flex;overflow-x:auto;gap:8px;padding:12px 24px;border-bottom:1px solid var(--dsw-alias-border-l1)}
.dsk-stat{border:1px solid var(--dsw-alias-border-l2);border-radius:8px;background:var(--dsw-alias-bg-layer-2);display:flex;min-width:116px;flex:1;flex-direction:column;align-items:flex-start;padding:10px 12px;text-align:left}
.dsk-stats button[aria-pressed=true]{border-color:var(--dsw-alias-brand-primary)}
.dsk-stats strong{font-size:22px;line-height:26px;font-variant-numeric:tabular-nums;letter-spacing:-.6px}.dsk-stats span{margin-top:2px;color:var(--dsw-alias-label-secondary);font-size:11px}
.dsk-filters{flex-shrink:0;display:flex;flex-wrap:wrap;align-items:flex-end;gap:12px;padding:12px 24px;border-bottom:1px solid var(--dsw-alias-border-l1);max-height:38vh;overflow:auto;background:var(--dsw-alias-bg-layer-1)}
...
.dsk-board-scroll{flex:1;min-height:0;overflow:auto;padding:20px 24px 28px;overscroll-behavior:contain;scroll-padding:20px}
.dsk-columns{display:grid;grid-template-columns:repeat(7,minmax(272px,1fr));gap:16px;min-width:2000px;align-items:start}
.dsk-column{min-height:180px;border:1px solid var(--dsw-alias-border-l1);border-radius:12px;background:color-mix(in srgb,var(--dsw-alias-label-primary) 3%,var(--dsw-alias-bg-base))}
.dsk-column>header{display:flex;align-items:center;justify-content:space-between;padding:14px 12px;border-bottom:1px solid var(--dsw-alias-border-l1)}
.dsk-column h2{margin:0;font-size:13px;font-weight:650}.dsk-column>header>span{min-width:22px;border-radius:10px;padding:2px 6px;background:var(--dsw-alias-bg-layer-3);color:var(--dsw-alias-label-secondary);font-size:11px;text-align:center}
.dsk-card-list{display:flex;flex-direction:column;gap:12px;padding:10px}
.dsk-show-more{width:100%;min-height:36px;border-style:dashed}
.dsk-card{border:1px solid var(--dsw-alias-border-l2);border-left:3px solid var(--dsh-card-status);border-radius:10px;padding:14px;background:var(--dsw-alias-bg-layer-2);cursor:pointer;box-shadow:0 1px 2px var(--dsw-alias-bg-mask-1)}
...
.dsk-card:hover{border-color:var(--dsw-alias-border-l3);border-left-color:var(--dsh-card-status)}
.dsk-card[draggable=true]{cursor:grab}.dsk-card-head{display:flex;align-items:flex-start;justify-content:space-between;gap:7px}.dsk-card-head strong{min-width:0;overflow-wrap:anywhere;font-size:14px;line-height:20px}
...
.dsk-card-badges{display:flex;flex-wrap:wrap;gap:4px;margin-top:8px}.dsk-card-badges span{border-radius:5px;padding:3px 5px;background:var(--dsw-alias-bg-layer-3);color:var(--dsw-alias-label-secondary);font-size:11px}.dsk-card-badges span[data-warning=true]{background:var(--dsw-alias-state-warn-tertiary);color:var(--dsw-alias-state-warn-label)}
...
.dsk-agents{display:flex;flex-direction:column;gap:6px;margin-top:8px;font-size:11px}.dsk-agents button{padding:5px;white-space:normal;text-align:left}.dsk-agent-filters{display:flex;flex-wrap:wrap;gap:4px}.dsk-agent-filters button[aria-pressed=true]{border-color:var(--dsw-alias-brand-primary)}.dsk-agent-list{display:flex;flex-direction:column;gap:8px;margin-top:8px}.dsk-agent-row{display:flex;flex-direction:column;gap:5px;border-left:2px solid var(--dsw-alias-border-l2);padding-left:8px;overflow-wrap:anywhere}.dsk-agent-row p{margin:0}.dsk-agent-row>span{color:var(--dsw-alias-label-secondary)}
...
.dsk-root .dsk-primary{background:var(--dsw-alias-brand-primary);color:var(--dsw-alias-label-primary-inverted);border-color:var(--dsw-alias-brand-primary);font-weight:600}
.dsk-stats .dsk-stat:not(button){border-color:transparent;background:transparent;min-width:94px;flex:.8}
.dsk-stats button[aria-pressed=true]{background:color-mix(in srgb,var(--dsw-alias-brand-primary) 8%,var(--dsw-alias-bg-base));box-shadow:inset 0 -2px var(--dsw-alias-brand-primary)}
.dsk-stat[data-status=running] strong{color:var(--dsw-alias-brand-primary)}.dsk-stat[data-status=waiting] strong{color:var(--dsw-alias-state-warn-label)}.dsk-stat[data-status=blocked] strong{color:var(--dsw-alias-state-error-primary)}
.dsk-view-options{flex-basis:100%;min-width:0}.dsk-view-options>summary{width:max-content;cursor:pointer;font-size:12px;font-weight:600;padding:6px 0;color:var(--dsw-alias-label-secondary)}
.dsk-options-grid{display:grid;grid-template-columns:repeat(4,minmax(130px,1fr));gap:12px;padding:12px 0 4px}.dsk-options-grid label{min-width:0}.dsk-options-grid .dsk-check{min-width:0}
.dsk-root summary:focus-visible,.dsk-agent-scroll:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:3px}
.dsk-columns[data-focused]{grid-template-columns:minmax(0,760px);min-width:0;justify-content:center}.dsk-column-empty{margin:20px 12px;color:var(--dsw-alias-label-tertiary);font-size:12px;text-align:center}
.dsk-agent-summary{display:flex;flex-direction:column;gap:8px;padding-top:10px;border-top:1px solid var(--dsw-alias-border-l1)}.dsk-agent-summary strong{font-size:12px;font-variant-numeric:tabular-nums}.dsk-agent-summary button{min-height:32px}
.dsk-agent-scroll{max-height:420px;overflow:auto;overscroll-behavior:contain;padding:0 4px 4px 0;scrollbar-gutter:stable}.dsk-agent-scroll .dsk-agent-list{margin-top:4px}
.dsk-agent-row{padding:10px 0 10px 10px;gap:4px;border-left-color:var(--dsw-alias-border-l2);border-bottom:1px solid var(--dsw-alias-border-l1)}
.dsk-root .dsk-agent-name{border:0;background:transparent;padding:0;font-weight:600;font-size:12px;line-height:18px;overflow-wrap:anywhere}
.dsk-agent-row>strong{font-size:10px;line-height:16px;display:flex;align-items:center;gap:5px}.dsk-agent-row>strong:before{content:'';height:6px;width:6px;border-radius:50%;background:var(--dsw-alias-label-tertiary)}
.dsk-agent-row[data-activity=running]>strong:before{background:var(--dsw-alias-state-success-primary)}.dsk-agent-row[data-activity=waiting]>strong:before{background:var(--dsw-alias-state-warn-primary)}
.dsk-agent-mode{font-size:10px}.dsk-agent-details{font-size:11px;color:var(--dsw-alias-label-secondary)}.dsk-agent-details>summary{cursor:pointer;padding:4px 0}.dsk-agent-details>div{display:flex;flex-direction:column;gap:5px;padding:6px 0;overflow-wrap:anywhere}
.dsk-agent-filters{display:flex;flex-wrap:wrap;gap:4px;padding:4px;border:1px solid var(--dsw-alias-border-l1);border-radius:8px;background:var(--dsw-alias-bg-base)}.dsk-agent-filters button{flex:1;min-width:65px;text-align:center;font-size:10px;border-color:transparent}.dsk-agent-filters button[aria-pressed=true]{background:color-mix(in srgb,var(--dsw-alias-brand-primary) 8%,var(--dsw-alias-bg-base));border-color:var(--dsw-alias-border-l2);font-weight:600}
@media(max-width:900px){.dsk-topbar{align-items:stretch;flex-direction:column}.dsk-topbar nav{justify-content:flex-start}.dsk-filters label{min-width:140px;flex:1}.dsk-filters .dsk-search{min-width:100%}.dsk-board-scroll{padding:16px}.dsk-stats .dsk-stat{min-width:110px}.dsk-options-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.dsk-columns{grid-template-columns:repeat(7,minmax(272px,78vw));min-width:max-content}}
@media(max-width:560px){.dsk-topbar,.dsk-stats,.dsk-filters{padding-left:12px;padding-right:12px}.dsk-topbar nav{gap:6px}.dsk-topbar nav button{min-height:38px}.dsk-filters{gap:8px}.dsk-filters label{min-width:calc(50% - 4px)}.dsk-view-options{min-width:100%}.dsk-options-grid{grid-template-columns:1fr}.dsk-modal-backdrop{padding:10px}.dsk-agent-scroll{max-height:45vh}}
